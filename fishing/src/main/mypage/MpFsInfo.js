...
const MpFsInfo = () => {
  ...
  const fetchSpotInfo = async () => {
    const res = await fetch(
      "http://localhost:8012/api/v1/fishing/getspotinfo",
      {
        method: "GET",
        headers: {
    ...
    <section className="MyPageMainBox">
      <div className="mainbox1">
        <div className="mychoicebox">
          ...
          <h1>
            {userInfo.userGrade === "OWNER" && <Link to={"/mpbt"}>배</Link>}
          </h1>
          <h1>
        ...
        <div className="userinfobox">
          <div className="profilebox">
            {spotInfo && spotInfo[0] ? (
              <img
                ...
                title="마이페이지"
                src={spotInfo.spotImageLocation[0]}
              />
            ) : (
              <img
                className="my-profile"
          ...
          <div className="namebox">
            {spotInfo && spotInfo[0] ? (
              <div className="nickName">{spotInfo[0].spotName}</div>
            ) : (
              <div className="nickName">등록된 낚시터가없습니다!</div>
            )}
            {spotInfo && spotInfo[0] ? (
              <div>{spotInfo.spotDescription}</div>
            ) : (
              <div>낚시터 정보를 등록해주세요!</div>
            )}
          </div>
          <div className="btbox">
            {spotInfo && spotInfo[0] ? (
              <>
                <button className="isbtn">
                  <Link to={"/myquery"}>글 삭제하기</Link>
                </button>
                <button className="isbtn">
                  <Link to={"/myquery"}>낚시터 정보 수정하기</Link>
                </button>
              </>
            ) : (
              <button className="isbtn">
                <Link to={"/myquery"}>글 등록하기</Link>
              </button>
            )}
