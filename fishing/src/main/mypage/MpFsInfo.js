    ...
    <section className="MyPageMainBox">
      <div className="mainbox1">
        ...
        <div className="userinfobox">
          ...
          {spotInfo && spotInfo[0]?(<img className="my-profile" title="마이페이지" src={spotInfo.spotImageLocation[0]} />) : (<img className="my-profile" title="마이페이지" src={require('./../icons/unknown.png')} />)}
          </div>
          <div className="namebox">
          {spotInfo && spotInfo[0] ? (<div className="nickName">{spotInfo[0].spotName}</div>) : (<div className="nickName">등록된  낚시터가없습니다!</div>)}
          {spotInfo && spotInfo[0] ? (<div>{spotInfo.spotDescription}</div>):(<div>낚시터 정보를 등록해주세요!</div>)}
          </div>
          <div className="btbox">
