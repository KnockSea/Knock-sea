      ...
      <div className="mainbox1">
        ...
        <div className="userinfobox">
          <div className="profilebox">
            {shipinfo.shipImageLocation &&
            shipinfo.shipImageLocation.length > 0 ? (
              <img
                className="my-profile"
                title="마이페이지"
                src={shipinfo.shipImageLocation[0]}
              />
            ) : (
              <img
                className="my-profile"
          ...
          <div className="namebox">
            <div className="nickName">
              {shipinfo.shipName ? (
                ...
                <span>등록된 배가 없습니다.</span>
              )}
            </div>
            <div>
              {shipinfo.shipDescription ? (
                <span>{shipinfo.shipDescription}</span>
              ) : (
                <span>배를 등록해주세요.</span>
              )}
            </div>
            <div>
              {shipinfo.shipLikeCount || (
                <span>{setShipinfo.shipLikeCount}</span>
              )}
            </div>
          </div>
          <div className="btbox">
            {shipinfo && shipinfo.shipId ? (
              <>
                <button className="isbtn" onClick={deleteShip}>
                  배 정보 삭제하기
                </button>
